...
let currentUser = localStorage.getItem('dm_current_user'); 
...
function hideAllViews() {
    // This removes the 'active' class from EVERY view, including Auth
...
function toggleAuthMode() {
    ...
    if (isLoginMode) {
        ...
        toggleText.innerHTML = `Don't have an account? <span id="btn-toggle-auth" style="color: var(--brand-gold); cursor: pointer; font-weight: 700;">Create one</span>`;
    } else {
        btn.innerText = "Create Account";
        toggleText.innerHTML = `Already have an account? <span id="btn-toggle-auth" style="color: var(--brand-gold); cursor: pointer; font-weight: 700;">Sign in</span>`;
    }
    // Re-attach listener to new dynamically created span
    document.getElementById('btn-toggle-auth').addEventListener('click', toggleAuthMode);
}
...
function updateUIForPro() {
    ...
    if (isProUser) { badge.classList.add('show'); } 
...
function exportBackup() {
    const historyStr = localStorage.getItem('tc_history'); if(!historyStr) { alert("No history to export."); return; }
    const history = JSON.parse(historyStr); if(history.length === 0) { alert("No history to export."); return; }

    let csvContent = "Project Name,Calculator,Result,Details,Date & Time\n";
    history.forEach(item => {
        const proj = `"${item.proj.replace(/"/g, '""')}"`; const title = `"${item.title.replace(/"/g, '""')}"`; const res = `"${item.res.replace(/"/g, '""')}"`; const sub = `"${item.sub.replace(/"/g, '""')}"`; const time = `"${item.time.replace(/"/g, '""')}"`;
    ...
    const a = document.createElement('a'); a.href = url; a.download = "DirtMath_Logs.csv"; a.click();
}
